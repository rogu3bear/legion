...
from libcst.metadata import PositionProvider, ScopeProvider, QualifiedNameProvider
...
class AgentInstantiationCodemod(VisitorBasedCodemodCommand):
    ...
    def __init__(self, context: CodemodContext) -> None:
        ...
        self.apply_fixes: bool = getattr(context.args, "apply", False)
        ...
        self.repo_root = getattr(context.args, "repo_root", Path.cwd())
    ...
    def should_skip_file(self) -> bool:
        ...
        try:
            ...
            orchestrator_abs_path = (self.repo_root / ORCHESTRATOR_FILE_PATH_STR).resolve()
    ...
    def visit_Call(self, node: cst.Call) -> None:
        ...
        if resolved_class_name:
            ...
            if warning_message not in self.warnings: # Avoid duplicate warnings for same node if visited multiple times
...
def main() -> None:
    ...
    for file_path in files_to_process:
        ...
        try:
            ...
            context = CodemodContext(args=args, filename=str(file_path.relative_to(args.repo_root) if file_path.is_absolute() and args.repo_root in file_path.parents else file_path))
            ...
            if args.apply:
                if input_tree.code != output_tree.code:
                    ...
                    print(f"Applied fixes to: {file_path.relative_to(args.repo_root) if file_path.is_absolute() and args.repo_root in file_path.parents else file_path}")
