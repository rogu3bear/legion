...
def parse_file(path: Path) -> List[Tuple[str, str, List[Tuple[str, List[str], str]]]]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef) and (
            ...
            if not role:  # If class docstring didn't provide a role
                for item in node.body:
                    if isinstance(item, ast.Assign):
                        for target in item.targets:
                            if isinstance(target, ast.Name) and target.id == 'system_prompt':
                                if isinstance(item.value, (ast.Constant, ast.Str)) and isinstance(item.value.s if isinstance(item.value, ast.Str) else item.value.value, str): # Support Str for older ast, Constant for newer
                                    prompt_value = item.value.s if isinstance(item.value, ast.Str) else item.value.value
                                    # Take the first non-empty line of system_prompt
                                    lines = [line.strip() for line in prompt_value.strip().splitlines()]
                                    first_meaningful_line = next((line for line in lines if line), None)
                                    if first_meaningful_line:
                                        role = first_meaningful_line
                                    break  # Found system_prompt
                        if role:  # If role was found from system_prompt
                            break

            if not role:  # If still no role
                role = 'No description'
