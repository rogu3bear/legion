...
class HandshakeTests(unittest.TestCase):
    ...
    def test_successful_handshake(self):
        resp1 = self.service.handle(
            {"phase": "INITIAL_REQUEST", "agent_id": "a1", "capabilities": []}
        )
        ...
        resp2 = self.service.handle(
            {"phase": "AUTH_RESPONSE", "agent_id": "a1", "signed_challenge": signed}
        )
        self.assertEqual(resp2["status"], "registered")

    def test_bad_signature(self):
        self.service.handle(
            {"phase": "INITIAL_REQUEST", "agent_id": "a2", "capabilities": []}
        )
        resp = self.service.handle(
            {"phase": "AUTH_RESPONSE", "agent_id": "a2", "signed_challenge": "bad"}
        )
