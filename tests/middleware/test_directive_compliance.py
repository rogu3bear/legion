...
class DirectiveComplianceTests(unittest.TestCase):
    def setUp(self):
        self.patch_feed = patch(
            "middleware.src.middleware.directive_compliance.post_agent_feed"
        )
    ...
    def test_compliant_request(self):
        status, details = self.dc.check(
            "summarize", {"task_id": "1"}, agent_id="researcher_agent"
        )
