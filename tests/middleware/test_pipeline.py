...
class PipelineTests(unittest.TestCase):
    ...
    def test_pipeline_success(self):
        ...
        with patch(
            "legion.middleware.therapist_validate",
            return_value={"valid": True, "directive": "ok"},
        ):
            result = run_middleware_pipeline(
                {"agent": "agent", "directive": "ok", "confidence": 0.9},
    ...
    def test_pipeline_invalid_directive(self):
        ...
        with patch(
            "legion.middleware.therapist_validate",
            return_value={"valid": True, "directive": "bad"},
        ):
            result = run_middleware_pipeline(
                {"agent": "agent", "directive": "bad", "confidence": 0.9},
                confidence_threshold=0.75,
            )
    ...
    def test_pipeline_low_confidence(self):
        ...
        with patch(
            "legion.middleware.therapist_validate",
            return_value={"valid": False, "reason": "low confidence"},
        ):
            result = run_middleware_pipeline(
                {"agent": "agent", "directive": "ok", "confidence": 0.5},
                confidence_threshold=0.75,
            )
        self.assertFalse(result["final_valid"])
