...
export default function EchoNexus() {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<"search" | "stream" | "stats">(
    "search",
  );
  ...
  const toggleStreaming = () => {
    if (isStreaming) {
      ...
      const eventSource = new EventSource(
        `/api/v1/echo/stream?${params.toString()}`,
      );
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 flex-1 min-w-0">
              <span
                className={`px-2 py-1 text-xs font-medium rounded ${getLevelColor(event.level)}`}
              >
                {event.level}
              </span>
          ...
          <div style={{ height: "400px" }}>
            {streamingEvents.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-500">
                {isStreaming
                  ? "Waiting for events..."
                  : "Start streaming to see live events"}
              </div>
            )}
          </div>
        </>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-yellow-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-yellow-600">
                  {stats.recent_activity.last_hour}
                </p>
              ...
              <div className="bg-gray-50 p-4 rounded-lg md:col-span-2">
                ...
                <div className="space-y-2">
                  {Object.entries(stats.events_by_level).map(
                    ([level, count]) => (
                      <div
                        key={level}
                      ...
                      >
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded ${getLevelColor(level)}`}
                        >
                          {level}
                        </span>
              ...
              <div className="bg-purple-50 p-4 rounded-lg">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {stats.active_agents.map((agent) => (
                    <div
                      key={agent}
                      className="text-sm text-purple-700 bg-purple-100 px-2 py-1 rounded"
                    >
                      {agent}
                    </div>
                  ))}
